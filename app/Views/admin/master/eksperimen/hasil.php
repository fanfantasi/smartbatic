<div class="col-12" id="container">
	<div class="ibox">
	    ...
	    <div class="ibox-body">
            ...
            <hr>
            <h4 class="m-b-20 font-strong" id="titlemateri"></h4> <hr>
...
<script type="text/javascript">
	...
	function showImages()
	{
		$.ajax({
			type:'POST',
			url:'<?= base_url('admin/geteksperimen')?>',
    ...
    $("#combomateri").combobox({
        onChange: function(value){
            $.ajax({
                ...
                url:'<?= base_url('admin/geteksperimen')?>',
                cache:false,
                data:'search_data='+$('#search').val()+'&sekolahid='+$('#combosekolah').combobox('getValue')+'&materiid='+value,
                beforeSend:function(){
                    if(!$(".loader").hasClass("is-active")){
                        $(".loader").addClass("is-active");
                    }
                ...
                success:function(response){
                    ...
                    $('#card').html(response)
                    var title = $('#combomateri').combobox('getText');
                    $('#titlemateri').html(title)
                },
                error:function(e){
                    if($(".loader").hasClass("is-active")){
                    $(".loader").removeClass("is-active");
        ...
        onChange: function(value){
            ...
            $.ajax({
                ...
                success:function(response){
                    ...
                    $('#card').html(response)
                    var title = $('#combomateri').combobox('getText');
                    $('#titlemateri').html(title==''?'Semua Materi':title)
                },
                error:function(e){
                    if($(".loader").hasClass("is-active")){
                    $(".loader").removeClass("is-active");
